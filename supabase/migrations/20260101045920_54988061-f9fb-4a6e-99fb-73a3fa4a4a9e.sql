-- Update cleanup function: pause (not end) chat sessions after 5 minutes of inactivity
...
AS $function$
BEGIN
  -- 1. PAUSE chat sessions idle for 5+ minutes (billing stops, but session stays open)
  UPDATE active_chat_sessions
  SET status = 'paused',
      end_reason = 'idle_5min_pause'
  WHERE status = 'active'
    AND last_activity_at < NOW() - INTERVAL '5 minutes';

  -- 2. Delete chat messages older than 10 MINUTES (includes selfies, audio, video, documents)
