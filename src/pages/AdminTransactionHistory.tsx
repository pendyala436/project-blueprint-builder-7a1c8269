...
const AdminTransactionHistory = () => {
  ...
  const loadAllData = async () => {
    try {
      ...
      const profileMap = new Map(profiles?.map(p => [p.user_id, p as { full_name: string | null; gender: string | null; country: string | null }]) || []);
      ...
      toast.error("Transactions unavailable", { description: "Unable to load transaction data. Please refresh." });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Complete platform transaction records</p>
            </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-8">
              ...
              <TabsTrigger value="wallet">Wallet Txns</TabsTrigger>
              <TabsTrigger value="earnings">Women Earnings</TabsTrigger>
                      ...
                      <TableBody>
                        {(() => {
                          ...
                          return sorted.map(tx => {
                            // transaction_type is the canonical column; fall back to type for old rows
                            const txType = (tx as any).transaction_type ?? tx.type;
                            const isDebit = txType === "debit" || txType === "withdrawal";
            ...
            {/* Wallet Transactions Tab — Men only (debits=charges, credits=recharges) */}
                        ...
                        return (
                          <TableRow key={tx.id}>
                            ...
                            <TableCell className="text-xs font-mono text-muted-foreground">
                              {(tx as any).session_id ? String((tx as any).session_id).slice(0, 8) + "…" : "—"}
                            </TableCell>
                            <TableCell className="max-w-48 truncate text-sm">{tx.description}</TableCell>
                            <TableCell className="text-sm font-medium">
                              {(tx as any).balance_after != null ? `₹${Number((tx as any).balance_after).toFixed(2)}` : "—"}
                            </TableCell>
                            <TableCell>{getStatusBadge(String((tx as any).status ?? 'completed'))}</TableCell>
            ...
            {/* Women's Earnings Tab — per-session credit rows */}
            <TabsContent value="earnings">
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Chat ₹2/min per man · Video ₹4/min · Group ₹2/min per man · Non-Indian women ₹0
                  </p>
                </CardHeader>
                <CardContent className="p-0">
                  <Table>
                    ...
                    <TableBody>
                      {womenEarnings.map(e => (
                        <TableRow key={e.id} className="hover:bg-muted/30">
                          ...
                          <TableCell className="text-xs font-medium">{e.user_name}</TableCell>
                          <TableCell>
                            <Badge variant="outline" className="text-[10px]">
                              {e.earning_type?.replace(/_/g, " ")}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-xs font-mono text-muted-foreground">
