...
const AdminFinanceReports = () => {
  ...
  const loadQueueStats = async () => {
    try {
      ...
      toast.error("Queue stats unavailable", { description: "Unable to load queue statistics. Please refresh." });
    }
  };
    ...
    try {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const uniqueUsers = new Set([
          ...(walletTxns || []).map((t: any) => t.user_id),
        ]);
      ...
      toast.error("Trend data unavailable", { description: "Unable to load trend data. Please refresh the page." });
    }
  };
  ...
  const loadReportData = async () => {
    ...
    try {
      ...
      const totalDeposits = creditTxns?.reduce((sum, t: any) => sum + Number(t.credit), 0) || 0;
      ...
      toast.error("Report unavailable", { description: ERROR_MESSAGES.admin.loadFailed });
    } finally {
      setLoading(false);
    }
  };
