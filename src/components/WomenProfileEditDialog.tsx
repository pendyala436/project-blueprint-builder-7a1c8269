/**
 * WomenProfileEditDialog Component
 * 
 * Profile edit dialog specifically for female users.
...
import { Loader2, Lock, User, Calendar, MapPin, Briefcase, Book, Heart, Camera, Languages, Globe, Check, ChevronsUpDown, Sparkles } from "lucide-react";
import { countries } from "@/data/countries";
import { statesByCountry, State } from "@/data/states";
import ProfilePhotosSection from "@/components/ProfilePhotosSection";
import { WOMEN_ALL_LANGUAGES, WOMEN_INDIAN_LANGUAGES, WOMEN_NON_INDIAN_LANGUAGES } from "@/data/women-languages";
...
interface WomenProfileEditDialogProps {
  open: boolean;
...
const INTEREST_OPTIONS = [
  "Music", "Movies", "Travel", "Reading", "Cooking", "Photography",
  "Art", "Fashion", "Fitness", "Dancing", "Yoga", "Meditation", "Nature", "Pets"
];

const LIFE_GOAL_OPTIONS = [
  "Career Growth", "Family", "Travel the World", "Financial Freedom",
  "Health & Fitness", "Education", "Creative Pursuits", "Finding Love", "Personal Development"
];

const WomenProfileEditDialog = ({ open, onOpenChange, onProfileUpdated }: WomenProfileEditDialogProps) => {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (data?.primary_language) {
        const foundLang = WOMEN_ALL_LANGUAGES.find(l => l.name === data.primary_language);
        if (foundLang) {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Edit Profile
            <Badge variant="secondary" className="ml-2 bg-pink-100 text-pink-700">Women</Badge>
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-6 py-4">
          {/* Protected Fields */}
          <div className="space-y-4 p-4 bg-pink-50/50 dark:bg-pink-950/20 rounded-lg border border-dashed border-pink-200 dark:border-pink-800">
          ...
          <div className="space-y-2">
            <Label className="flex items-center gap-2">
              ...
              Profile Photos
              <Badge variant="outline" className="ml-1 text-xs border-pink-300">Important for visibility</Badge>
            </Label>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Choose your native language. You'll be matched with users who speak this language or auto-translation will be enabled.
            </p>
            <Popover open={languageOpen} onOpenChange={setLanguageOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" role="combobox" className="w-full justify-between border-pink-200 dark:border-pink-800">
                  {userLanguage ? (
              ...
              <PopoverContent className="w-full p-0" align="start">
                <Command>
                  ...
                  <CommandList className="max-h-60">
                    ...
                    <CommandGroup heading={`🇮🇳 Indian Languages (${WOMEN_INDIAN_LANGUAGES.length})`}>
                      {WOMEN_INDIAN_LANGUAGES
                    ...
                    <CommandGroup heading={`🌍 International Languages (${WOMEN_NON_INDIAN_LANGUAGES.length})`}>
                      {WOMEN_NON_INDIAN_LANGUAGES
                        .filter(l => l.name.toLowerCase().includes(languageSearch.toLowerCase()) ||
                          (l.nativeName && l.nativeName.toLowerCase().includes(languageSearch.toLowerCase())))
                        .map((lang) => (
                          <CommandItem
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button onClick={handleSave} disabled={isSaving || !hasChanges} className="bg-pink-500 hover:bg-pink-600">
...
export default WomenProfileEditDialog;
