...
const ChatEarningsDisplay = ({ 
  ...
  useEffect(() => {
    ...
    inactivityInterval.current = setInterval(async () => {
      ...
      // Background cleanup - non-critical, session will timeout naturally
        }
      ...
      <div className="flex items-center gap-4 px-4 py-2 text-sm bg-muted/50 border-b border-border">
        ...
        <div className="flex items-center gap-1.5">
          <Circle className={cn(
            ...
            userStatus === "online" && "text-online",
            userStatus === "busy" && "text-warning",
        ...
        <div className={cn(
          ...
          activeChats >= 3 ? "bg-warning/20 text-warning" : "bg-muted text-muted-foreground"
        )}>
  ...
  return (
    <div className="flex items-center gap-4 px-4 py-2 text-sm bg-primary/10 border-b border-primary/20">
      ...
      {earningBadgeType === "star" && (
        <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/20">
          ...
          <span className="text-xs font-medium text-primary">Earner</span>
        </div>
      )}
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="font-medium text-primary">₹{currentSessionEarnings.toFixed(2)}</span>
      </div>
      ...
      <div className="flex items-center gap-1.5 ml-auto">
        ...
        <span className="font-bold text-primary">₹{todayEarnings.toFixed(0)}</span>
      </div>

      {/* Active Chats Badge */}
      <div className={cn(
        "px-2 py-0.5 rounded-full text-xs font-medium",
        activeChats >= 3 ? "bg-warning/20 text-warning" : "bg-primary/20 text-primary"
      )}>
        {activeChats}/3 active
      </div>
    </div>
