...
const SCRIPT_PATTERNS: Array<{
  ...
  { regex: /[\u0F00-\u0FFF]/u, language: 'Tibetan', script: 'Tibetan', nllbCode: 'bod_Tibt' },
  ...
  // Santali (Ol Chiki)
  { regex: /[\u1C50-\u1C7F]/u, language: 'Santali', script: 'Ol Chiki', nllbCode: 'sat_Olck' },
...
// European language patterns (Latin script languages)
const LATIN_LANGUAGE_PATTERNS: Array<{
  language: string;
  nllbCode: string;
  charPatterns: RegExp[];
  wordPatterns: RegExp[];
  ...
  // Spanish
  {
    ...
    nllbCode: 'spa_Latn',
    charPatterns: [/[ñ¿¡]/u],
    wordPatterns: [/\b(que|como|cuando|donde|por|para|esta|pero|muy|con|los|las|una|del|más)\b/i],
  },
  // French
  {
    language: 'French',
    nllbCode: 'fra_Latn',
    charPatterns: [/[àâäçéèêëîïôùûüÿœæ]/u],
    wordPatterns: [/\b(je|tu|il|elle|nous|vous|est|sont|avoir|être|dans|pour|avec|que|qui|les|des|une|sur)\b/i],
  },
  // German
  {
    language: 'German',
    nllbCode: 'deu_Latn',
    charPatterns: [/[äöüß]/u],
    wordPatterns: [/\b(ich|du|er|sie|wir|ihr|ist|sind|haben|sein|und|oder|aber|mit|für|das|die|der|den)\b/i],
  },
  // Portuguese
  {
    language: 'Portuguese',
    nllbCode: 'por_Latn',
    charPatterns: [/[ãõçáàâéêíóôú]/u],
    wordPatterns: [/\b(que|como|quando|onde|por|para|esta|mas|muito|com|não|você|são|tem|uma|dos|das)\b/i],
  },
  // Italian
  {
    language: 'Italian',
    nllbCode: 'ita_Latn',
    charPatterns: [/[àèéìíòóùú]/u],
    wordPatterns: [/\b(che|come|quando|dove|perché|per|questa|ma|molto|con|non|sono|sei|siamo|della|degli|uno)\b/i],
  },
  // Vietnamese
  {
    language: 'Vietnamese',
    nllbCode: 'vie_Latn',
    charPatterns: [/[àáảãạăắằẳẵặâấầẩẫậèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữựỳýỷỹỵđ]/u],
    wordPatterns: [/\b(là|có|của|và|để|này|những|được|không|một|trong|với|cho|người|về)\b/i],
  },
  // Turkish
  {
    language: 'Turkish',
    nllbCode: 'tur_Latn',
    charPatterns: [/[ğışçöüİ]/u],
    wordPatterns: [/\b(bir|bu|ne|ve|ile|için|var|yok|evet|hayır|merhaba|nasıl|kim|nerede|gibi|olarak)\b/i],
  },
  // Indonesian
  {
    language: 'Indonesian',
    nllbCode: 'ind_Latn',
    charPatterns: [],
    wordPatterns: [/\b(apa|bagaimana|kapan|dimana|siapa|mengapa|dan|atau|tapi|dengan|untuk|ini|itu|yang|adalah|tidak|bisa)\b/i],
  },
  // Polish
  {
    language: 'Polish',
    nllbCode: 'pol_Latn',
    charPatterns: [/[ąćęłńóśźżĄĆĘŁŃÓŚŹŻ]/u],
    wordPatterns: [/\b(jest|nie|co|jak|gdzie|kiedy|dlaczego|ale|tak|to|się|są|być|mieć|dla)\b/i],
  },
  // Dutch
  {
    language: 'Dutch',
    nllbCode: 'nld_Latn',
    charPatterns: [],
    wordPatterns: [/\b(de|het|een|is|zijn|van|en|in|met|voor|op|naar|als|maar|ook|niet|dat|dit|die)\b/i],
  },
  // Romanian
  {
    language: 'Romanian',
    nllbCode: 'ron_Latn',
    charPatterns: [/[ăâîșț]/u],
    wordPatterns: [/\b(este|sunt|nu|da|ce|cum|unde|când|de|și|în|cu|pentru|la|pe|sau)\b/i],
  },
  // Czech
  {
    language: 'Czech',
    nllbCode: 'ces_Latn',
    charPatterns: [/[áčďéěíňóřšťúůýžÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ]/u],
    wordPatterns: [/\b(je|jsou|co|jak|kde|kdy|proč|ale|ano|ne|to|se|být|mít|pro|na)\b/i],
  },
  // Hungarian
  {
    language: 'Hungarian',
    nllbCode: 'hun_Latn',
    charPatterns: [/[áéíóöőúüű]/u],
    wordPatterns: [/\b(egy|van|nem|igen|mi|hogy|hol|mikor|miért|de|és|ez|az|vagy|ami|aki)\b/i],
  },
  // Swedish
  {
    language: 'Swedish',
    nllbCode: 'swe_Latn',
    charPatterns: [/[åäöÅÄÖ]/u],
    wordPatterns: [/\b(är|har|och|att|det|en|som|inte|för|på|med|av|till|den|var)\b/i],
  },
  // Danish
  {
    language: 'Danish',
    nllbCode: 'dan_Latn',
    charPatterns: [/[æøåÆØÅ]/u],
    wordPatterns: [/\b(er|har|og|at|det|en|som|ikke|for|på|med|af|til|den|jeg)\b/i],
  },
  // Norwegian
  {
    language: 'Norwegian',
    nllbCode: 'nob_Latn',
    charPatterns: [/[æøåÆØÅ]/u],
    wordPatterns: [/\b(er|har|og|at|det|en|som|ikke|for|på|med|av|til|den|jeg|kan)\b/i],
  },
  // Finnish
  {
    language: 'Finnish',
    nllbCode: 'fin_Latn',
    charPatterns: [/[äöÄÖ]/u],
    wordPatterns: [/\b(on|ja|ei|kyllä|mitä|miten|missä|milloin|miksi|mutta|olla|että|tämä|se|minä)\b/i],
  },
  // Swahili
  {
    language: 'Swahili',
    nllbCode: 'swh_Latn',
    charPatterns: [],
    wordPatterns: [/\b(ni|na|ya|wa|kwa|katika|kwamba|lakini|hii|hiyo|ndiyo|hapana|habari|jambo|asante)\b/i],
  },
  // Hausa
  {
    language: 'Hausa',
    nllbCode: 'hau_Latn',
    charPatterns: [/[ɓɗƙ]/u],
    wordPatterns: [/\b(ne|ce|da|ko|amma|wannan|wancan|a|i|ina|sannu|na'am|a'a|yaya)\b/i],
  },
  // Yoruba
  {
    language: 'Yoruba',
    nllbCode: 'yor_Latn',
    charPatterns: [/[ẹọṣ]/u],
    wordPatterns: [/\b(ni|ti|ati|tabi|ṣugbọn|eyi|iyẹn|bẹẹni|rara|bawo|kini|nibo)\b/i],
  },
];
...
/**
 * Detect Latin-script languages (European, African, etc.)
 */
function detectLatinLanguage(text: string): LanguageDetectionResult | null {
  const lowerText = text.toLowerCase();
  
  // Check each Latin language pattern
  for (const pattern of LATIN_LANGUAGE_PATTERNS) {
    let score = 0;
    
    // Check character patterns
    for (const charPattern of pattern.charPatterns) {
    ...
    // Check word patterns
    for (const wordPattern of pattern.wordPatterns) {
    ...
    if (score >= 2) {
      return {
        ...
        nllbCode: pattern.nllbCode,
        script: 'Latin',
        confidence: Math.min(score / 8, 0.95),
        isPhonetic: false,
        isLatinScript: true,
      };
    }
