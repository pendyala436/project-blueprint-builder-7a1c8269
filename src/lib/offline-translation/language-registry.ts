/**
 * Offline Translation - Dynamic Language Registry
 * ================================================
 * 
 * Builds language database dynamically from languages.ts.
 * NO hardcoded language names - all data-driven.
 * 
 * Features:
 * - Language lookup by name, code, or native name
 * - Script detection and classification
 * - Dialect fallback mapping
...
function initializeDatabase(): void {
  languages.forEach((lang: Language) => {
    ...
    // Map ASCII-compatible native names
...
// ============================================================
// SCRIPT FALLBACK MAP - For dialect routing
// ============================================================
...
/**
 * Normalize language name - all lookups are dynamic
 */
 ...
 */
export function detectLanguage(text: string): { language: string; confidence: number; isLatin: boolean } {
...
export function getEffectiveLanguage(lang: string): string {
  ...
  if (SUPPORTED_PHRASE_LANGUAGES.has(normalized)) {
    return normalized;
  }
...
export function getLanguagesByScript(script: string): LanguageInfo[] {
  return Array.from(languageByName.values()).filter(
