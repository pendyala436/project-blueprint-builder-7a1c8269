...
export function useOptimizedAuth() {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      ...
      toast.error('Authentication error', { description: classifyError(error, 'initialize your session').message });
        if (mounted) {
          setState(prev => ({
            ...prev,
            isLoading: false,
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      ...
      // Context fetch failure - will navigate to dashboard with fallback
            }
          }, 0);
        } else {
          setState({
