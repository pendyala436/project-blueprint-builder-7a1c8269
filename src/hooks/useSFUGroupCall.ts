...
export function useSFUGroupCall({
  ...
  // ICE servers for WebRTC (STUN + optional self-hosted TURN — no paid services)
  // Configured via VITE_TURN_URL / VITE_TURN_USERNAME / VITE_TURN_CREDENTIAL env vars
  ...
  const createPeerConnection = useCallback((participantId: string) => {
    const pc = new RTCPeerConnection(ICE_SERVERS_SFU);
  ...
  const goLive = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Could not go live', { description: ERROR_MESSAGES.video.startFailed });
      setState(prev => ({ 
        ...prev, 
        isConnecting: false, 
  ...
  const joinStream = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Could not join stream', { description: ERROR_MESSAGES.video.joinFailed });
      setState(prev => ({ 
        ...prev, 
        isConnecting: false, 
